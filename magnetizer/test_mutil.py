...
def test_link_h1():
    ...
    assert link_h1("<h1>link</h1>" + trailing_html, url) == "<h1><a href='/link.html'>link</a></h1>" + trailing_html

    # Any other html before the <h1> should be ignored but h1 should still get a link
    assert link_h1(leading_html + "<h1>link</h1>" + trailing_html, url) == leading_html + "<h1><a href='/link.html'>link</a></h1>" + trailing_html
...
def test_downgrade_headings():
    ...
    assert downgrade_headings(input) == expected
...
def test_first_image_url_from_html():
    ...
    assert first_image_url_from_html(example_with_no_image) == None
